$VMs = Get-AzureRmVM | Sort-Object -Property Name
$ListNIC = @()
    if($VMs){
    foreach($VM in $VMs){
        foreach($nic in $VM.NetworkProfile.NetworkInterfaces){
            ...
            if($vnic.IpConfigurations.PublicIpAddress.Id){
                ...
                $publicIpAddress = Get-AzureRmPublicIpAddress -ResourceGroupName $vnic.ResourceGroupName -Name $publicIpName
            }
            $Vnicfinal = [pscustomobject]@{
                ...
                VnicPublicIpAddressName = $publicIpAddress.Name
                VnicPublicIpAddress = $publicIpAddress.IpAddress
                VnicPublicIpAddressSKU = $publicIpAddress.Sku.Name
